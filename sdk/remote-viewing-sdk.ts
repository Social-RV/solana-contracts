...
export class RemoteViewingSDK {
  ...
  async verifySessionIntegrity(sessionId: string): Promise<{
    ...
    // Verify target index calculation (accounting for excluded indices)
    const completedIndices = sessionData.completedTargetIndices || [];
    
    // Create array of available indices (excluding completed targets)
    const availableIndices: number[] = [];
    for (let i = 0; i < poolData.targets.length; i++) {
      if (!completedIndices.includes(i)) {
        availableIndices.push(i);
      }
    }
    
    // Calculate the filtered index from blockhash
    const filteredIndex = this.hashToIndex(
      sessionData.submissionBlockhash,
      availableIndices.length,
    );
    
    // Get the actual target index from the available indices
    const calculatedIndex = availableIndices[filteredIndex];
